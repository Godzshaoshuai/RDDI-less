...
class MVN_DDI(nn.Module):
    def __init__(self, in_features, hidd_dim, kge_dim, rel_total, heads_out_feat_params, blocks_params):
        ...
        for i, (head_out_feats, n_heads) in enumerate(zip(heads_out_feat_params, blocks_params)):
            print("i",i)
            print("head_out_feats",head_out_feats)
            print("n_heads",n_heads)
            block = MVN_DDI_Block(n_heads, in_features, head_out_feats, final_out_feats=self.hidd_dim,edge_attr_dim=n_heads*in_features)
...
class IntraGraphAttention(nn.Module):
    def __init__(self, input_dim, edge_attr_dim):
        super().__init__()
        self.edge_emb = nn.Linear(edge_attr_dim, input_dim)
        self.intra = GATv2Conv(input_dim, 32, heads=2, edge_dim=input_dim)

    def forward(self, data):
        x = F.elu(data.x)
        edge_attr_proj = self.edge_emb(data.edge_attr)
        intra_rep = self.intra(x, data.edge_index, edge_attr=edge_attr_proj)
        return intra_rep
...
class InterGraphAttention(nn.Module):
    def __init__(self, input_dim, edge_attr_dim):
        super().__init__()
        self.edge_emb = nn.Linear(edge_attr_dim, input_dim)
        self.inter = GATv2Conv((input_dim, input_dim), 32, heads=2, edge_dim=input_dim)

    def forward(self, h_data, t_data, b_graph):
        edge_attr_proj = self.edge_emb(b_graph.edge_attr)
        h_input = F.elu(h_data.x)
        t_input = F.elu(t_data.x)
        t_rep = self.inter((h_input, t_input), b_graph.edge_index, edge_attr=edge_attr_proj)
        h_rep = self.inter((t_input, h_input), b_graph.edge_index[[1, 0]], edge_attr=edge_attr_proj)
        return h_rep, t_rep

class MVN_DDI_Block(nn.Module):
    def __init__(self, n_heads, in_features, head_out_feats, final_out_feats, edge_attr_dim):
        super().__init__()
        self.feature_conv = GATv2Conv(in_features, head_out_feats, heads=n_heads, edge_dim=in_features)
        #self.edge_emb = nn.Linear(edge_attr_dim, in_features)
        self.intraAtt = IntraGraphAttention(head_out_feats * n_heads, edge_attr_dim)
        self.interAtt = InterGraphAttention(head_out_feats * n_heads, edge_attr_dim)
        self.readout = SAGPooling(n_heads * head_out_feats, min_score=-1)

    def forward(self, h_data, t_data, b_graph):
        #edge_attr_proj_h = self.edge_emb(h_data.edge_attr)
        #edge_attr_proj_t = self.edge_emb(t_data.edge_attr)

        #h_data.x = self.feature_conv(h_data.x, h_data.edge_index, edge_attr=edge_attr_proj_h)
        #t_data.x = self.feature_conv(t_data.x, t_data.edge_index, edge_attr=edge_attr_proj_t)
        h_data.x = self.feature_conv(h_data.x, h_data.edge_index, edge_attr=h_data.edge_attr)
        t_data.x = self.feature_conv(t_data.x, t_data.edge_index, edge_attr=t_data.edge_attr)
