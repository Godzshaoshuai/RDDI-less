...
df_drugs_smiles = pd.read_csv('drugbank/drug_smiles.csv')
...
df_all_pos_ddi = pd.read_csv('drugbank/ddis.csv')
...
class DrugDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        for h, t, r in batch:
            pos_rels.append(r)
            h_data = self.__create_graph_data(h)#RCGRL可以接受的图结构数据
    ...
    def __create_graph_data(self, id):
        ...
        #print("edge_index weidu ",edge_index.dim())
        #print("输出1",edge_index.size(0))
        #print("输出2", edge_index.size(-1))
        num_edges = edge_index.size(-1)
        edge_attr = torch.ones((num_edges, 1), dtype=torch.float)  # [num_edges, 1]
